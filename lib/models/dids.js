...
class Dids {
  ...
  */
  ...
  _setSummary (results) {
    ...
    for (const key of keys) {
      ...
      items[key].sum = items[key].values.reduce((a, b) => {
  ...
  async _setSelector () {
    try {
      ...
      if (this._isMultipleWordQuery(this.filters.q)) {
        ...
        const regexp = new RegExp(this.filters.q, 'i')
        selector.text = {
          $regex: regexp
        }
  ...
  */
   ...
   * @return Objecty
   */
  _getSummaryObject (keys, array) {
    return Object.assign(...keys.map(key => {

      // get the value of each object
      let val = array.map(item => item[key]).filter(item => item)
